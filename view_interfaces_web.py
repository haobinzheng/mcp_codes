...
@app.route("/api/router_data")
def route_api_router_data():
    date = request.args.get("date", "")
    ...
    if not os.path.exists(router_path):
        return jsonify({"timestamps": [], "interfaces": {}, "series": {}})
...
@app.route("/api/high_utilization")
def route_api_high_utilization():
    date = request.args.get("date", "")
    if not date or not re.match(r"^\d{4}-\d{2}-\d{2}$", date):
        return jsonify({"high_interfaces": []})
    ...
    return jsonify({"high_interfaces": high_items})

def main() -> None:
    print(f"GFiber Interface Audit Dashboard running on http://{WEB_HOST}:{WEB_PORT}")
