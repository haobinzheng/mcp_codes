...
<body>
  ...
  <script>
    ...
    function renderP95Data() {
      ...
      if (p95ViewMode === 'cards') {
        ...
        lastP95Data.high_interfaces_history.forEach(item => {
          ...
          const activeDays = [];
          for (let idx = 0; idx < times.length; idx++) {
            if (inputs[idx] !== null || outputs[idx] !== null) {
              activeDays.push({
                dateStr: times[idx],
                inVal: inputs[idx] !== null ? inputs[idx] : 0,
                outVal: outputs[idx] !== null ? outputs[idx] : 0
              });
            }
          }
          ...
          tr0.style.borderTop = '2px solid var(--border)';
          tr0.innerHTML = `
            <td rowspan="${N}" style="font-weight: 600; color: var(--accent-purple); vertical-align: top;">${item.router}</td>
            <td rowspan="${N}" style="font-family: 'JetBrains Mono', monospace; font-weight: 600; vertical-align: top;">${item.interface}</td>
            <td rowspan="${N}" style="vertical-align: top; color: var(--text-muted);">${item.neighbor}</td>
            <td rowspan="${N}" style="vertical-align: top; color: var(--text-muted);">${item.speed}</td>
            <td rowspan="${N}" style="vertical-align: top;"><span class="badge ${item.is_400g_upgraded ? 'badge-ok' : 'badge-warn'}">${item.upgrade_status}</span></td>
            <td style="color: var(--text-muted); font-size: 13px; font-weight: 600;">${activeDays[0].dateStr}</td>
            <td><span class="badge ${activeDays[0].inVal > 80 ? 'badge-high' : 'badge-warn'}">${Math.round(activeDays[0].inVal)}%</span></td>
            <td><span class="badge ${activeDays[0].outVal > 80 ? 'badge-high' : 'badge-warn'}">${Math.round(activeDays[0].outVal)}%</span></td>
          `;
          ...
          for (let i = 1; i < N; i++) {
            const tr = document.createElement('tr');
            tr.innerHTML = `
              <td style="color: var(--text-muted); font-size: 13px; font-weight: 600;">${activeDays[i].dateStr}</td>
              <td><span class="badge ${activeDays[i].inVal > 80 ? 'badge-high' : 'badge-warn'}">${Math.round(activeDays[i].inVal)}%</span></td>
              <td><span class="badge ${activeDays[i].outVal > 80 ? 'badge-high' : 'badge-warn'}">${Math.round(activeDays[i].outVal)}%</span></td>
            `;
            tbody.appendChild(tr);
          }
        });
    ...
    function exportP95ToCSV() {
      ...
      const headers = ["Router", "Interface", "Neighbor", "Speed", "Upgrade Status", "Date", "Peak Input %", "Peak Output %"];
      const csvRows = [headers.join(",")];
        ...
        for (let idx = 0; idx < times.length; idx++) {
          ...
          if (isFirst) {
            row = [
              ...
              escapeCSV(item.upgrade_status),
              escapeCSV(dateStr),
              escapeCSV(Math.round(inVal) + "%"),
              escapeCSV(Math.round(outVal) + "%")
            ];
            isFirst = false;
          } else {
            row = [
