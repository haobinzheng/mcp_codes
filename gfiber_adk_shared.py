...
11. For Rancid-backed device inventories: use ``list_rancid_device_families`` when the user asks which / how many **device families** exist (keys from ``rancid_folders``, e.g. juniper, cisco). Use ``list_rancid_devices`` for hostnames under one family; for **function categories** within a family use ``list_function_categories=True`` (live depot only). For "all dr devices" use ``hostname_prefix="dr"``. For **Juniper models, platforms, or OS/Junos version from saved configs** (including "audit all the cr devices os version" meaning **read Rancid** for each ``cr*`` host), use ``list_rancid_juniper_platform_models`` with ``hostname_prefix="cr"``; for **all MX960 / EX4200-48t / …** hosts use ``model_substring="mx960"`` or ``model_substring="ex4200-48t"`` (substring on the parsed ``Model:`` line, case-insensitive). Live depot only; raise ``max_files`` if the read budget stops before the full depot is scanned. The ``junos`` field is the parsed release—use ``start_audit_run`` only when the user wants **fresh** CLI output.
